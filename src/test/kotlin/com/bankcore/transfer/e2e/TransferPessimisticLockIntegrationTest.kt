...
@TestPropertySource(properties = ["transfer.lock.strategy=PESSIMISTIC"])
class TransferPessimisticLockIntegrationTest : TestcontainersIntegrationBase() {
    ...
    @Test
    fun `비관적 락 전략에서 동일 출금 계좌 동시 송금 시 정합성을 유지한다`() {
        ...
        assertEquals(INITIAL_BALANCE - (TRANSFER_AMOUNT * successCount.toBigDecimal()), latestFrom.balance)
        assertEquals(TRANSFER_AMOUNT * successCount.toBigDecimal(), latestTo.balance)
